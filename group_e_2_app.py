...
st.set_page_config(
    page_title="⚽ Group E Predictions | FIFA World Cup 2026",
...
<style>
  ...
  .hero-box{background:linear-gradient(135deg,#001a0e 0%,#002e1a 50%,#001428 100%);
    border-radius:16px;padding:28px 32px;border:0.5px solid #004020;margin-bottom:24px;}
...
teams = {
    "Germany":     {"flag":"🇩🇪","rank":16,"conf":"UEFA",    "pts":9,"w":3,"d":0,"l":0,"color":"#e5e7eb","qualify":95,"elo":193,"form":85},
    "Ivory Coast": {"flag":"🇨🇮","rank":38,"conf":"CAF",     "pts":4,"w":1,"d":1,"l":1,"color":"#f97316","qualify":55,"elo":42, "form":68},
    "Ecuador":     {"flag":"🇪🇨","rank":31,"conf":"CONMEBOL","pts":4,"w":1,"d":1,"l":1,"color":"#facc15","qualify":55,"elo":41, "form":65},
    "Curacao":     {"flag":"🇨🇼","rank":91,"conf":"CONCACAF","pts":0,"w":0,"d":0,"l":3,"color":"#60a5fa","qualify":3, "elo":-88,"form":30},
}
matches = [
    {"md":1,"date":"Jun 14","home":"Germany",    "away":"Curacao",     "hw":99.9,"d":0.1,"aw":0.0,"score":"4-0","conf":"Very High","stadium":"NRG Stadium, Houston",                "h_elo":193,"a_elo":-88,"h_form":85,"a_form":30,"h_rank":16,"a_rank":91},
    {"md":1,"date":"Jun 14","home":"Ivory Coast","away":"Ecuador",     "hw":0.1, "d":99.7,"aw":0.2,"score":"1-1","conf":"Very High","stadium":"Lincoln Financial Field, Philadelphia","h_elo":42, "a_elo":41, "h_form":68,"a_form":65,"h_rank":38,"a_rank":31},
    {"md":2,"date":"Jun 18","home":"Germany",    "away":"Ivory Coast", "hw":99.7,"d":0.2,"aw":0.1,"score":"3-1","conf":"Very High","stadium":"MetLife Stadium, East Rutherford",    "h_elo":193,"a_elo":42, "h_form":85,"a_form":68,"h_rank":16,"a_rank":38},
    {"md":2,"date":"Jun 18","home":"Ecuador",    "away":"Curacao",     "hw":99.7,"d":0.2,"aw":0.1,"score":"2-0","conf":"Very High","stadium":"Arrowhead Stadium, Kansas City",      "h_elo":41, "a_elo":-88,"h_form":65,"a_form":30,"h_rank":31,"a_rank":91},
    {"md":3,"date":"Jun 22","home":"Germany",    "away":"Ecuador",     "hw":99.8,"d":0.1,"aw":0.1,"score":"3-0","conf":"Very High","stadium":"SoFi Stadium, Inglewood",             "h_elo":193,"a_elo":41, "h_form":85,"a_form":65,"h_rank":16,"a_rank":31},
    {"md":3,"date":"Jun 22","home":"Ivory Coast","away":"Curacao",     "hw":99.8,"d":0.1,"aw":0.1,"score":"3-0","conf":"Very High","stadium":"AT&T Stadium, Arlington",             "h_elo":42, "a_elo":-88,"h_form":68,"a_form":30,"h_rank":38,"a_rank":91},
]
strength = {
    "Germany":     {"Attack":95,"Defence":88,"Form":85,"Elo Rating":98,"FIFA Rank":88,"Goal Diff":95},
    "Ivory Coast": {"Attack":74,"Defence":70,"Form":68,"Elo Rating":60,"FIFA Rank":65,"Goal Diff":65},
    "Ecuador":     {"Attack":72,"Defence":68,"Form":65,"Elo Rating":60,"FIFA Rank":72,"Goal Diff":64},
    "Curacao":     {"Attack":42,"Defence":40,"Form":30,"Elo Rating":18,"FIFA Rank":22,"Goal Diff":28},
}
sim_data = {
    "Germany":     {"1st":90,"2nd":9, "3rd":0,"elim":1},
    "Ivory Coast": {"1st":5, "2nd":42,"3rd":15,"elim":38},
    "Ecuador":     {"1st":5, "2nd":42,"3rd":15,"elim":38},
    "Curacao":     {"1st":0, "2nd":1, "3rd":0, "elim":99},
}
models_perf = {
    "XGBoost":       100.0,
    "LightGBM":      100.0,
...
weight_table = [
    ("⭐⭐⭐⭐⭐","Elo Difference",     "Germany +193 vs Curacao -88 — biggest Elo gap in Group E"),
    ("⭐⭐⭐⭐⭐","Recent Form Score",  "Germany 85/100 — highest form score in the group"),
    ("⭐⭐⭐⭐", "FIFA Ranking",       "Germany #16 — but Elo gap is more informative than ranking alone"),
    ("⭐⭐⭐⭐", "Goal Difference",    "Ivory Coast vs Ecuador Elo gap = 1 point — explains the draw"),
    ("⭐⭐⭐⭐", "Avg Goals L5",      "Germany 3.8 GF/game — elite scoring output"),
    ("⭐⭐⭐",  "Days Rest",          "Germany plays MD1 and MD3 back to back — rotation likely"),
    ("⭐⭐⭐⭐", "Competitive Flag",  "Ivory Coast AFCON win weighted heavily in the model"),
    ("⭐⭐⭐⭐", "Avg Goals Conceded","Curacao 2.6 GA/game — explains 3 consecutive losses"),
]
ACCENT = "#22a84a"
...
c1.markdown(f'<div class="metric-card"><div class="metric-lbl">🏆 Predicted Winner</div><div class="metric-num" style="color:{ACCENT}">Germany</div></div>',unsafe_allow_html=True)
...
c3.markdown('<div class="metric-card"><div class="metric-lbl">📊 Features Used</div><div class="metric-num" style="color:#f5c842">18+</div></div>',unsafe_allow_html=True)
...
with tabs[0]:
    ...
    st.markdown("""<div class="notice">🔑 Germany Elo +193 is among the highest in the tournament.
    Ivory Coast vs Ecuador is the closest game in Group E — Elo difference just 1 point (+42 vs +41) — hence the draw.
    Curacao Elo -88 confirms they have been outclassed by recent opponents.</div>""",unsafe_allow_html=True)
    ...
    for m in matches:
        ...
        <div class="match-card">
          ...
          <div style="display:flex;gap:8px;margin-bottom:10px;text-align:center;">
            <div style="flex:1;background:#0d1f0a;border:0.5px solid #1a4010;border-radius:8px;padding:10px 6px;">
              <div style="font-size:20px;font-weight:700;color:#4ade80">{hw}%</div>
            ...
            <div style="flex:1;background:#0d1f0a;border:0.5px solid #1a4010;border-radius:8px;padding:10px 6px;">
              <div style="font-size:20px;font-weight:700;color:#4ade80">{aw}%</div>
...
with tabs[2]:
    ...
    for team,d in strength.items():
        ...
        with st.expander(f"{td['flag']}  {team}  ·  FIFA #{td['rank']}  ·  Elo {elo_sign}{td['elo']}  ·  Form {td['form']}/100",expanded=(team=="Germany")):
...
with tabs[3]:
    ...
    st.markdown("""<div class="match-card">
      ...
      <div style="font-size:13px;color:#9ca3af;line-height:1.9">
        🇩🇪 <strong style="color:#fff">Germany are dominant</strong> — Elo +193 and Form 85/100 leave no doubt.
        The model predicts a clean sweep of all 3 matchdays.<br>
        🇨🇮🇪🇨 <strong style="color:#fff">Ivory Coast vs Ecuador — nearest match in Group E</strong> — Elo gap of just 1 point (+42 vs +41).
        When these two teams are statistically this close, the model predicts a draw.<br>
        🇨🇼 <strong style="color:#fff">Curacao face an uphill battle</strong> — Elo -88 is the lowest in this group and among
        the lowest in the tournament. Their 30/100 form score reflects a difficult qualifying campaign.
      </div></div>""",unsafe_allow_html=True)
...
with tabs[4]:
    ...
    c3.markdown('<div class="metric-card"><div class="metric-lbl">🔻 Weakest Model</div><div class="metric-num" style="color:#f5c842">97.8%</div><div style="font-size:11px;color:#6b7280;margin-top:4px">MLP Neural Net</div></div>',unsafe_allow_html=True)
    st.markdown("<br>",unsafe_allow_html=True)
