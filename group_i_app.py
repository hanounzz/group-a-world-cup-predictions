...
st.set_page_config(
    page_title="⚽ Group I Predictions | FIFA World Cup 2026",
...
<style>
  ...
  .hero-box{background:linear-gradient(135deg,#00142a 0%,#002244 50%,#001020 100%);
    border-radius:16px;padding:28px 32px;border:0.5px solid #003060;margin-bottom:24px;}
  .hero-eyebrow{font-size:11px;letter-spacing:.1em;color:#60aef5;text-transform:uppercase;margin-bottom:4px;}
  ...
  .stTabs [aria-selected="true"]{background:#1f2937 !important;color:#60aef5 !important;font-weight:600;}
...
# ── DATA (from actual match files + weighted ML pipeline) ─────────────────────
teams = {
    "France":  {"flag":"🇫🇷","rank":2, "conf":"UEFA","pts":7,"w":2,"d":1,"l":0,"color":"#3b82f6","qualify":92,"elo":119,"form":87},
    "Senegal": {"flag":"🇸🇳","rank":17,"conf":"CAF", "pts":7,"w":2,"d":1,"l":0,"color":"#22a84a","qualify":88,"elo":80, "form":87},
    "Norway":  {"flag":"🇳🇴","rank":47,"conf":"UEFA","pts":1,"w":0,"d":1,"l":2,"color":"#ef4444","qualify":12,"elo":13, "form":71},
    "Iraq":    {"flag":"🇮🇶","rank":58,"conf":"AFC", "pts":1,"w":0,"d":1,"l":2,"color":"#f5c842","qualify":8, "elo":-20,"form":71},
}
matches = [
    {"md":1,"date":"Jun 16","home":"France", "away":"Senegal","hw":0.3, "d":99.4,"aw":0.3,"score":"2-2","conf":"Medium",   "stadium":"MetLife Stadium, East Rutherford","h_elo":119,"a_elo":80, "h_form":87,"a_form":87,"h_rank":2, "a_rank":17},
    {"md":1,"date":"Jun 16","home":"Iraq",   "away":"Norway", "hw":0.3, "d":99.4,"aw":0.3,"score":"1-2","conf":"Medium",   "stadium":"Gillette Stadium, Foxborough",     "h_elo":-20,"a_elo":13, "h_form":71,"a_form":71,"h_rank":58,"a_rank":47},
    {"md":2,"date":"Jun 20","home":"France", "away":"Iraq",   "hw":76.5,"d":21.2,"aw":2.3,"score":"2-1","conf":"High",     "stadium":"AT&T Stadium, Arlington",         "h_elo":119,"a_elo":-20,"h_form":87,"a_form":71,"h_rank":2, "a_rank":58},
    {"md":2,"date":"Jun 20","home":"Norway", "away":"Senegal","hw":3.0, "d":26.8,"aw":70.2,"score":"2-2","conf":"High",    "stadium":"Estadio BBVA, Monterrey",         "h_elo":13, "a_elo":80, "h_form":71,"a_form":87,"h_rank":47,"a_rank":17},
    {"md":3,"date":"Jun 24","home":"France", "away":"Norway", "hw":73.2,"d":24.1,"aw":2.7,"score":"2-2","conf":"High",    "stadium":"Mercedes-Benz Stadium, Atlanta",  "h_elo":119,"a_elo":13, "h_form":87,"a_form":71,"h_rank":2, "a_rank":47},
    {"md":3,"date":"Jun 24","home":"Senegal","away":"Iraq",   "hw":73.5,"d":23.9,"aw":2.6,"score":"2-1","conf":"High",    "stadium":"Hard Rock Stadium, Miami",        "h_elo":80, "a_elo":-20,"h_form":87,"a_form":71,"h_rank":17,"a_rank":58},
]
strength = {
    "France":  {"Attack":88,"Defence":85,"Form":87,"Elo Rating":80,"FIFA Rank":98,"Goal Diff":82},
    "Senegal": {"Attack":85,"Defence":90,"Form":87,"Elo Rating":72,"FIFA Rank":86,"Goal Diff":84},
    "Norway":  {"Attack":65,"Defence":64,"Form":71,"Elo Rating":48,"FIFA Rank":56,"Goal Diff":62},
    "Iraq":    {"Attack":52,"Defence":58,"Form":71,"Elo Rating":38,"FIFA Rank":46,"Goal Diff":55},
}
sim_data = {
    "France":  {"1st":58,"2nd":34,"3rd":0,"elim":8},
    "Senegal": {"1st":42,"2nd":46,"3rd":0,"elim":12},
    "Norway":  {"1st":0, "2nd":12,"3rd":8,"elim":80},
    "Iraq":    {"1st":0, "2nd":8, "3rd":4,"elim":88},
}
models_perf = {"XGBoost":100.0,"LightGBM":100.0,"ExtraTrees":100.0,"Random Forest":100.0,"SVM":100.0,"MLP":97.5}
weight_table = [
    ("⭐⭐⭐⭐⭐","Elo Difference",     "France +119 vs Senegal +80 — just 39 points apart, hence the draw"),
    ("⭐⭐⭐⭐⭐","Recent Form Score",  "France 87 = Senegal 87 — statistically identical, strongest draw signal"),
    ("⭐⭐⭐⭐", "FIFA Ranking",       "France #2 vs Senegal #17 — ranking gap visible but form/Elo overrides"),
    ("⭐⭐⭐⭐", "Goal Difference",    "Senegal better GA (0.8 vs 1.2) — narrowly edges attack-defence balance"),
    ("⭐⭐⭐⭐", "Avg Goals Scored",   "Senegal 2.6 GF/game — highest in the group, great attacking record"),
    ("⭐⭐⭐",  "Days Rest",          "France and Senegal meet MD1 with equal rest — no advantage either way"),
    ("⭐⭐⭐⭐", "Competitive Flag",   "France WC qualifying, Nations League — full competitive record weighted"),
    ("⭐⭐⭐⭐", "Possession %",      "France 58% vs Senegal 56% — slight tactical edge for France"),
]
ACCENT = "#3b82f6"
...
<div class="hero-box">
  ...
  <div class="hero-title">⚽ Group I Predictions</div>
  <div class="hero-sub">France · Senegal · Norway · Iraq &nbsp;|&nbsp; 6 Matches · 3 Matchdays</div>
</div>""", unsafe_allow_html=True)

c1,c2,c3,c4 = st.columns(4)
c1.markdown(f'<div class="metric-card"><div class="metric-lbl">🏆 Predicted Winners</div><div class="metric-num" style="color:{ACCENT};font-size:22px">Fr & Sen</div></div>',unsafe_allow_html=True)
...
c3.markdown('<div class="metric-card"><div class="metric-lbl">🔥 Form Both Top 2</div><div class="metric-num" style="color:#f5c842">87/100</div></div>',unsafe_allow_html=True)
c4.markdown('<div class="metric-card"><div class="metric-lbl">⚡ MD1 Draw</div><div class="metric-num" style="color:#60aef5">99.4%</div></div>',unsafe_allow_html=True)
...
with tabs[0]:
    ...
    st.markdown("""<div class="notice">🔑 <strong>Group I is the most balanced top-2 in the tournament.</strong>
    France and Senegal both score 87/100 on Form — statistically identical. Their Elo difference (+119 vs +80)
    is only 39 points, making MD1 a genuine draw. Senegal's defensive record (0.8 GA/game) is actually
    <em>better</em> than France's (1.2 GA/game). FIFA ranking separates them on paper only — the data says otherwise.</div>""",unsafe_allow_html=True)
...
with tabs[1]:
    ...
    for m in matches:
        ...
        closest = '<span class="chip chip-blue">🔥 Closest match of tournament</span>' if ht=="France" and at=="Senegal" else ""
        st.markdown(f"""
        <div class="match-card">
          ...
          <div style="display:flex;gap:8px;margin-bottom:10px;text-align:center;">
            <div style="flex:1;background:#0d1e35;border:0.5px solid #1a3060;border-radius:8px;padding:10px 6px;">
              <div style="font-size:20px;font-weight:700;color:#60aef5">{hw}%</div>
          ...
          {win_chip} <span class="chip chip-gold">Confidence: {m['conf']}</span> {closest}
        </div>""", unsafe_allow_html=True)
...
with tabs[2]:
    ...
    for team,d in strength.items():
        ...
        with st.expander(f"{td['flag']}  {team}  ·  FIFA #{td['rank']}  ·  Elo {elo_sign}{td['elo']}  ·  Form {td['form']}/100",expanded=(team=="France")):
...
with tabs[3]:
    ...
    for i,(team,s) in enumerate(sim_data.items()):
        ...
        chip_html = (f'<span class="chip chip-blue">✅ Advances</span>' if qual_total > 70 else
    ...
    st.markdown("""<div class="match-card">
      ...
      <div style="font-size:13px;color:#9ca3af;line-height:1.9">
        🇫🇷🇸🇳 <strong style="color:#fff">France vs Senegal MD1 — most balanced opening match in the tournament</strong> —
        identical form scores (87/87), Elo gap of just 39. Model gives 99.4% draw probability.
        Goal difference across the group separates 1st from 2nd.<br>
        🇸🇳 <strong style="color:#fff">Senegal's defence is exceptional</strong> — only 0.8 GA/game, better than France's 1.2.
        Senegal beat Norway in MD2 despite Norway having higher FIFA ranking. Form and Elo carry the day.<br>
        🇮🇶🇳🇴 <strong style="color:#fff">Iraq vs Norway MD1 is also a near-draw</strong> — both teams score 71/100 on form,
        Elo gap is only 33 points. Best 3rd-place race possible for Norway.
      </div></div>""",unsafe_allow_html=True)
...
with tabs[4]:
    ...
    c3.markdown('<div class="metric-card"><div class="metric-lbl">📊 Total Metrics</div><div class="metric-num" style="color:#22a84a">18+</div><div style="font-size:11px;color:#6b7280;margin-top:4px">All from your files</div></div>',unsafe_allow_html=True)
    st.markdown("<br>",unsafe_allow_html=True)
    ...
    st.markdown("""<div class="notice">⚠️ Predictions computed from your actual match history files.
    Elo Difference and Recent Form Score carry 5x weight each. FIFA Ranking, Goal Difference, Competitive Match Flag
    and Avg Goals carry 4x weight. Group I result col is <em>Result (W/D/L)</em> — handled correctly by the pipeline.</div>""",unsafe_allow_html=True)

st.markdown("""<br>
<div style="text-align:center;font-size:11px;color:#374151;padding:16px 0;border-top:0.5px solid #1f2937;">
  ⚽ FIFA World Cup 2026 · Group I Predictor · Data from group_i_matches_history.xlsx &nbsp;|&nbsp; For fans, by fans
</div>""",unsafe_allow_html=True)
