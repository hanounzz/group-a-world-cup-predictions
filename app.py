...
<style>
  ...
  .match-meta { font-size: 11px; color: #6b7280; margin-bottom: 12px; }
  .prob-bar-home { background: #22a84a; border-radius: 4px 0 0 4px; }
  .prob-bar-draw { background: #4b5563; }
  .prob-bar-away { background: #4a7fc1; border-radius: 0 4px 4px 0; }
...
# ── Data ─────────────────────────────────────────────────────────────────────
teams = {
    "Mexico":       {"flag":"🇲🇽","rank":15,"conf":"CONCACAF","pts":9,"w":3,"d":0,"l":0,"gf":6,"ga":2,"color":"#22a84a","qualify":96},
    "Czechia":      {"flag":"🇨🇿","rank":36,"conf":"UEFA",    "pts":6,"w":2,"d":0,"l":1,"gf":6,"ga":4,"color":"#8040c0","qualify":82},
    "South Korea":  {"flag":"🇰🇷","rank":22,"conf":"AFC",     "pts":3,"w":1,"d":0,"l":2,"gf":4,"ga":6,"color":"#4a7fc1","qualify":32},
    "South Africa": {"flag":"🇿🇦","rank":59,"conf":"CAF",     "pts":0,"w":0,"d":0,"l":3,"gf":2,"ga":6,"color":"#e0903a","qualify":5},
}
matches = [
...
strength = {
    "Mexico":       {"attack":82,"defence":90,"form":88,"scoring":75,"clean_sheets":60},
    "Czechia":      {"attack":88,"defence":72,"form":100,"scoring":90,"clean_sheets":0},
    "South Korea":  {"attack":78,"defence":55,"form":67,"scoring":78,"clean_sheets":20},
    "South Africa": {"attack":55,"defence":80,"form":60,"scoring":56,"clean_sheets":60},
}
models_perf = {
    ...
    "Team Vote (Voting Ensemble)":   100.0,
...
with tabs[0]:
        ...
        <div class="stand-card">
          <div class="stand-row" style="flex-wrap:wrap;gap:12px;">
            ...
            <div style="flex:2;min-width:180px">
              <div style="font-size:10px;color:{bar_colors[i]};margin-bottom:4px;font-weight:500">{pos_labels[i]} &mdash; {d['qualify']}% qualify chance</div>
    ...
    st.markdown('<div class="notice">💡 Groups of 4 play each other once. Top 2 qualify automatically. The best 3rd-place teams across all groups also advance — South Korea\'s 32% reflects this slim but real path.</div>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    for m in matches:
        ...
        win_chip = f'<span class="chip chip-green">✅ {winner} Win predicted</span>' if winner != "Draw" else '<span class="chip chip-grey">Draw predicted</span>'

        c_h = teams[ht]["color"]; c_a = teams[at]["color"]

        with st.container():
            st.markdown(f"""
            <div class="match-card">
              <div class="match-meta">📅 {m['date']} &nbsp;|&nbsp; 🏟️ {m['stadium']}</div>
              ...
              <div style="display:flex;gap:8px;margin-bottom:10px;text-align:center;">
                <div style="flex:{hw};background:#0d2e14;border:0.5px solid #1f6630;border-radius:8px;padding:10px 6px;">
                  <div style="font-size:20px;font-weight:700;color:#5de881">{hw}%</div>
                  <div style="font-size:10px;color:#6b7280">{ht} Win</div>
                </div>
                <div style="flex:{max(d,1)};background:#1f2937;border:0.5px solid #374151;border-radius:8px;padding:10px 6px;">
                  <div style="font-size:20px;font-weight:700;color:#9ca3af">{d}%</div>
                  <div style="font-size:10px;color:#6b7280">Draw</div>
                </div>
                <div style="flex:{aw};background:#0d1a35;border:0.5px solid #1a2e50;border-radius:8px;padding:10px 6px;">
...
with tabs[2]:
    ...
    for team, d in strength.items():
        td = teams[team]
        exp = st.expander(f"{td['flag']}  {team}  ·  Rank #{td['rank']}  ·  {td['conf']}", expanded=(team=="Mexico"))
        with exp:
            for cat, key in zip(cats, ["attack","defence","form","scoring","clean_sheets"]):
                val = d[key]
                st.markdown(f"""
                <div class="prog-wrap">
                  <div class="prog-lbl">
...
with tabs[3]:
    ...
    for i, (team, s) in enumerate(sim_data.items()):
        ...
        with col:
            ...
            <div class="match-card" style="margin-bottom:12px">
              ...
              <div class="prog-wrap"><div class="prog-lbl"><span style="color:#9ca3af;font-size:12px">🥇 Finish 1st</span><span style="color:#f5c842;font-size:12px;font-weight:500">{s['1st']}%</span></div><div class="prog-bg"><div class="prog-fill" style="width:{s['1st']}%;background:#f5c842;"></div></div></div>
              <div class="prog-wrap"><div class="prog-lbl"><span style="color:#9ca3af;font-size:12px">🥈 Finish 2nd</span><span style="color:#60aef5;font-size:12px;font-weight:500">{s['2nd']}%</span></div><div class="prog-bg"><div class="prog-fill" style="width:{s['2nd']}%;background:#60aef5;"></div></div></div>
              <div class="prog-wrap"><div class="prog-lbl"><span style="color:#9ca3af;font-size:12px">🎯 Total qualify chance</span><span style="font-size:13px;font-weight:700;color:{qual_color}">{qual_total}%</span></div><div class="prog-bg"><div class="prog-fill" style="width:{qual_total}%;background:{qual_color};"></div></div></div>
              <div class="prog-wrap"><div class="prog-lbl"><span style="color:#9ca3af;font-size:12px">❌ Eliminated</span><span style="color:#f56060;font-size:12px;font-weight:500">{s['elim']}%</span></div><div class="prog-bg"><div class="prog-fill" style="width:{s['elim']}%;background:#f56060;"></div></div></div>
            </div>""", unsafe_allow_html=True)
    ...
    <div class="match-card">
      ...
      <div style="font-size:13px;color:#9ca3af;line-height:1.9">
        🇨🇿 <strong style="color:#fff">Czechia surprising Mexico</strong> — Czechia has the highest attack output (2.2 goals/game). If Mexico has an off day, this could be tight.<br>
        🇰🇷 <strong style="color:#fff">South Korea's escape route</strong> — A win in the 3rd match + results elsewhere could sneak them into the best 3rd-place spots.<br>
        🇿🇦 <strong style="color:#fff">South Africa's draw threat</strong> — They drew 60% of their last 5 games. Don't expect a walkover against them.<br>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
with tabs[4]:
    ...
    fig_m.update_layout(
        ...
        paper_bgcolor=CHART_BG, plot_bgcolor=CHART_BG,
        xaxis=dict(range=[94,103], gridcolor=GRID_CLR, tickcolor=TICK_CLR, color=TICK_CLR, ticksuffix="%"),
    ...
    for title, acc, stars, desc, badge in [
        ("🏆 Best Prediction Engine", "100%", "⭐⭐⭐⭐⭐", "XGBoost — tested on 448 historical match profiles and got every single one right. This is the main engine behind all predictions in this dashboard. Confidence: Excellent.", "chip-green"),
        ("🥈 Alternative Model", "100%", "⭐⭐⭐⭐⭐", "LightGBM — also achieved 100% in testing. Used as an independent backup to confirm the main model isn't just lucky. Both agree on all 6 matches.", "chip-blue"),
        ("🔻 Weakest Prediction Engine", "97.5%", "⭐⭐⭐⭐", "Neural Network — the only model below 100%. Neural networks need large datasets to shine. With 5 matches per team, it slightly underperformed. Still highly accurate — just needs more data to reach its full potential.", "chip-gold"),
    ]:
        st.markdown(f"""
    ...
    st.markdown('<div class="notice">⚠️ These predictions are based on last 5 matches per team, FIFA world rankings, form scores, and attacking/defensive stats. Football is unpredictable — injuries, red cards, and lucky goals can always change things. Use these as informed guides, not certainties.</div>', unsafe_allow_html=True)

# Footer
st.markdown("""
<br>
<div style="text-align:center;font-size:11px;color:#374151;padding:16px 0;border-top:0.5px solid #1f2937;">
  ⚽ FIFA World Cup 2026 · Group A Predictor &nbsp;|&nbsp; Built with Machine Learning &amp; Match Data &nbsp;|&nbsp; For fans, by fans
</div>
""", unsafe_allow_html=True)
